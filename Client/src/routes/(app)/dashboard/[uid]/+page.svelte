<script>
	...
	import { removeItem } from '$lib/firebase';
...
>
	<div class="h-full w-full bg-primary-900 p-16 py-48 text-center opacity-30">
		<h2 class="cursive text-2xl font-bold text-white md:text-6xl">Dashboard</h2>
	</div>
</div>
<SignedIn let:user>
	<div class="p-8">
		...
		<div class="my-4">
			<Button href="/dashboard/createItem">Create Item</Button>
		</div>
		<div
						...
						{#each data.items as item}
							<tr>
								...
								<td class="dark:border-strokedark border-b border-[#eee] px-4 py-5">
									<a href="/category/tag" class="text-black dark:text-white">{item.category}</a>
								</td>
								<td class="dark:border-strokedark border-b border-[#eee] px-4 py-5">
									<p
